...
import {
  simulateVerifyZProofOnchain,
  submitVerifyZProofOnchain,
  verifyZProofOnchain,
} from "../soroban/zproofSoroban.service.js";
...
export type SubmitZProofParams = {
  challengeId: string;
  wallet: string;
  appId?: string;
  score: number;
...
export async function createAndPrepareZProofV1(params: CreateZProofParams) {
  ...
  const xdr = await simulateVerifyZProofOnchain({
    proof,
    publicSignals,
...
export async function submitAndVerifyZProofV1(params: SubmitZProofParams) {
  const appId = params.appId || process.env.ZPROOF_APP_ID || "zproof-id-demo";
  ...
  const { proof, publicSignals } = await generateZProofV1(input);
  const onchainVerification = await submitVerifyZProofOnchain({
