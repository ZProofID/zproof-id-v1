...
async function prepareVerificationHumanityAttempt({
  ...
  if (!response.ok) {
    const error = await response.json().catch(() => null);
    throw new Error(error?.error || "Failed to verify humanity attempt");
  }

  const networkKey = "TESTNET";
  const signedXdr = await WalletKitService.signTx(data.xdr?.xdr, {
...
async function verifyHumanityAttempt({
  wallet,
  challengeId,
  results,
  finalPayload,
}) {
  const response = await fetch(`${HUMANITY_API_URL}/api/humanity/verify`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      wallet,
...
export default function HumanityGame() {
  ...
  useEffect(() => {
    ...
    async function verify() {
      ...
      try {
        const signedXdr = await prepareVerificationHumanityAttempt({
          wallet: connectedWallet,
        ...
        setBackendError(error.message || "Could not verify humanity attempt");
      } finally {
        setVerificationLoading(false);
      }
    }
          ...
          {!walletConnected ? (
            ...
            <>
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-4 shadow-2xl shadow-blue-950/30 backdrop-blur">
                ...
                {/* 
                <div
                  className="relative overflow-hidden rounded-[1.5rem] border border-white/10 bg-slate-950"
