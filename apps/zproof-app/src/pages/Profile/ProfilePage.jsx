...
function getZkProofSummary(rawZkProof) {
  ...
  return {
    verified:
      ...
      false,
    status:
      rawZkProof?.onchainVerification?.status ||
      rawZkProof?.status ||
      "UNKNOWN",
...
async function fetchWalletRecord(wallet) {
  if (!wallet) return null;

  const response = await fetch(
    `${HUMANITY_API_URL}/api/humanity/wallet/${encodeURIComponent(wallet)}`
  );

  const data = await response.json().catch(() => null);

  if (!response.ok) {
    throw new Error(data?.error || "Failed to fetch wallet verification");
  }
...
function normalizeRecord(record, wallet) {
  ...
  const attestation = record.latestAttestation ?? null;
  ...
  return {
    ...
    passed: record.latestPassed ?? message.passed ?? false,
    verified: record.latestPassed ?? message.passed ?? false,
    confidenceLevel: record.latestConfidenceLevel ?? message.confidenceLevel,

    averageHumanityScore:
      record.latestAverageHumanityScore ??
      record.latestResult?.averageHumanityScore,

    adjustedHumanityScore:
      record.latestAdjustedHumanityScore ??
      record.latestResult?.adjustedHumanityScore ??
      message.adjustedHumanityScore,

    averageAccuracy:
      record.latestAverageAccuracy ?? record.latestResult?.averageAccuracy,

    gamesPassed:
      record.latestGamesPassed ??
      record.latestResult?.gamesPassed ??
      message.gamesPassed,

    totalGames:
      record.latestTotalGames ??
      record.latestResult?.totalGames ??
      message.totalGames,

    issuedAt: message.issuedAt ?? record.createdAt,
    expiresAt: message.expiresAt,
    challengeId:
      record.latestChallengeId ??
      record.latestResult?.challengeId ??
      message.challengeId,

    botRiskSignals:
      record.latestBotRiskSignals ??
      record.latestResult?.botRiskSignals ??
      (message.botRiskScore !== undefined
        ? { botRiskScore: message.botRiskScore }
        : null),

    varianceAnalytics:
      record.latestVarianceAnalytics ?? record.latestResult?.varianceAnalytics,

    attestation,
    zkProof: record.latestZkProof,
    results: record.latestResult?.results ?? [],
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    async function loadRecord() {
      ...
      try {
        const nextRecord = await fetchWalletRecord(userKey);
        if (!cancelled) setRecord(nextRecord);
      } catch (error) {
        if (!cancelled) {
          setRecord(null);
          setRecordError(error.message || "Could not load wallet record");
        }
      } finally {
  ...
  const attestation = useMemo(
    () => normalizeRecord(record, userKey),
    [record, userKey]
  );
  ...
  const credentialPassed = Boolean(zkProof?.credentialPassed || isVerified);
  const fullVerified = isVerified && zkVerified;
  ...
  const wallet = attestation?.wallet ?? userKey ?? "Not connected";

  const verificationCount = attestation?.verificationCount ?? 0;
        ...
        <section className="mt-10">
          <div className="mb-6 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            ...
            >
              ...
              {recordLoading ? "Loading" : statusLabel}
            </div>
          </div>
...
function LoadingState() {
  return (
    <section className="rounded-[2rem] border border-white/10 bg-[#080B18]/80 p-8 shadow-2xl shadow-blue-950/30 backdrop-blur">
      <div className="flex items-center gap-3 text-slate-300">
        <Loader2 className="h-5 w-5 animate-spin text-blue-300" />
        Loading wallet verification record...
      </div>
    </section>
  );
}
